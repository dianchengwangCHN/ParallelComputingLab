// Use "gcc -O3 -fopenmp" to compile for gcc
// Use "icc -O3 -fopenmp" to compile for icc
...
void Merge(a, b, lo, mid, hi) int a[], b[], lo, mid, hi;
{
  int h, i, j, k;
  h = lo;
  i = lo;
...
void Merge_Sort_Par(int a[], int b[], int n, int nThreads)
{
  ...
  int T = 256;  //Chunk size
  // To be modified to create a parallel merge sort
  // Divide array into chunks and merge sort each chunk in parallel
  #pragma omp parallel
  {
    #pragma omp for schedule(guided, 1024)
    for (int i = 0; i < n; i += T)
    {
      Merge_Sort(a, b, i, i + T - 1 < n ? i + T - 1 : n - 1);
    }
  }
  // Merge the chunks until the whole array is sorted
  for (int i = 2 * T; i <= n * 2; i *= 2)
  {
    ...
    for (int j = 0; j < n; j += i)
    {
      ...
      Merge(a, b, j, j + i / 2 - 1, j + i - 1 < n ? j + i - 1 : n - 1);
    }
  }
}

void Test_Sorted(a, lo, hi) int a[], lo, hi;
{
  int i, notsorted;
  notsorted = 0;
  for (i = lo + 1; i <= hi; i++)
