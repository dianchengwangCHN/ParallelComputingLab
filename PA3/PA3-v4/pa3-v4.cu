...
int main(){
  ...
  dim3 grid(n / threads.x, n / threads.y / 2);
...
// You can make any changes; only requirement is that correctness test passes
  ...
  j = blockIdx.y * blockDim.y * 2 + threadIdx.y;

  int tx = threadIdx.x, ty = threadIdx.y;
  __shared__ double Bb0[SIZE * SIZE], Bb1[SIZE * SIZE], Ab[SIZE * SIZE];
  ...
  for (int ks = 0; ks < N; ks += SIZE) {
    ...
    __syncthreads();
    for (k = 0; k < SIZE; k++) {
      double temp = Ab[k * SIZE + tx];
      sum0 += temp * Bb0[ty * SIZE + k];
      sum1 += temp * Bb1[ty * SIZE + k];
    }
    __syncthreads();
  }
  C[j * N + i] = sum0;
