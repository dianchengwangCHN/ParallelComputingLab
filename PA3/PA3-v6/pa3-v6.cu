...
int main(){
  ...
  dim3 grid(n / threads.x / 2, n / threads.y / 4);
...
// You can make any changes; only requirement is that correctness test passes
  ...
  j = blockIdx.y * blockDim.y * 4 + ty;

  __shared__ double 
    Bb0[SIZE * SIZE], Bb1[SIZE * SIZE],
    Bb2[SIZE * SIZE], Bb3[SIZE * SIZE],
  ...
  for (int ks = 0; ks < N; ks += SIZE) {
    // Coalesced access
    Bb0[ty * SIZE + tx] = B[(ks + ty) * N + blockIdx.y * blockDim.y * 4 + tx];
    Bb1[ty * SIZE + tx] = B[(ks + ty) * N + blockIdx.y * blockDim.y * 4 + tx + SIZE];
    Bb2[ty * SIZE + tx] = B[(ks + ty) * N + blockIdx.y * blockDim.y * 4 + tx + SIZE * 2];
    Bb3[ty * SIZE + tx] = B[(ks + ty) * N + blockIdx.y * blockDim.y * 4 + tx + SIZE * 3];
    Ab0[ty * SIZE + tx] = A[(ks + ty) * N + i];
    Ab1[ty * SIZE + tx] = A[(ks + ty) * N + i + SIZE];
    __syncthreads();
    for (k = 0; k < SIZE; k++) {
      double Atmp0 = Ab0[k * SIZE + tx], Atmp1 = Ab1[k * SIZE + tx];
      double Btmp0 = Bb0[k * SIZE + ty], Btmp1 = Bb1[k * SIZE + ty];
      double Btmp2 = Bb2[k * SIZE + ty], Btmp3 = Bb3[k * SIZE + ty];
      sum0 += Atmp0 * Btmp0;  // C[j][i]
      sum1 += Atmp1 * Btmp0;  // C[j][i + SIZE]
      sum2 += Atmp0 * Btmp1;  // C[j + SIZE][i]
      sum3 += Atmp1 * Btmp1;  // C[j + SIZE][i + SIZE]
      sum4 += Atmp0 * Btmp2;  // C[j + SIZE * 2][i]
      sum5 += Atmp1 * Btmp2;  // C[j + SIZE * 2][i + SIZE]
      sum6 += Atmp0 * Btmp3;  // C[j + SIZE * 3][i]
      sum7 += Atmp1 * Btmp3;  // C[j + SIZE * 3][i + SIZE]
    }
    __syncthreads();
  }
  C[j * N + i] = sum0;
  ...
  C[(j + SIZE * 2) * N + i] = sum4;
  C[(j + SIZE * 2) * N + SIZE + i] = sum5;
  C[(j + SIZE * 3) * N + i] = sum6;
  C[(j + SIZE * 3) * N + SIZE + i] = sum7;
}
