...
int main(int argc, char *argv[])
{
  ...
  t2 = rtclock();
  printf("Base version: %.2f GFLOPs (approx.); Time = %.2f\n", 1.0e-9 * N * N * N / (t2 - t1), t2 - t1);
...
// OpenMP parallelization
  ...
  t1 = rtclock();
  // Loop permutation to make access stride better
  #pragma omp parallel private(i, j, k)
  {
    // Better trade-off between workload balance and overhead
  ...
  t2 = rtclock();
  printf("Optimized/parallelized version: %.2f GFLOPs (approx); Time = %.2f\n", 1.0e-9 * N * N * N / (t2 - t1), t2 - t1);
  compare(N, c, cc);
}
double rtclock(void)
{
