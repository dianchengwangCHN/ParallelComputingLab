...
int main(){
  ...
  dim3 grid(n / threads.x / 2, n / threads.y);
...
// You can make any changes; only requirement is that correctness test passes
  ...
  __shared__ double Bb[SIZE * SIZE], Ab[SIZE * SIZE * 2];

  double sum0 = 0, sum1 = 0;
  for(int ks = 0; ks < N; ks += SIZE){
    Bb[ty * SIZE + tx] = B[(ks + tx) * N + j];
    Ab[ty * SIZE * 2 + tx] = A[(ks + ty) * N + i];
    Ab[ty * SIZE * 2 + tx + SIZE] = A[(ks + ty) * N + i + SIZE];
    __syncthreads();
    for(k = 0; k < SIZE; k++){
      double temp = Bb[ty * SIZE + k];
      sum0 += Ab[k * SIZE * 2 + tx] * temp;
      sum1 += Ab[k * SIZE * 2 + tx + SIZE] * temp;
    }
    __syncthreads();
  }
  C[j * N + i] = sum0;
